...
cfstat_locs = ['/opt/ColdFusion9/bin/cfstat',
...
class ColdFusion:
    ...
    def run(self):
        ...
        self.checksLogger.debug(
            'ColdFusion: Checking for cfstat path in config')
        if 'Main' not in self.rawConfig and \
                        'coldfusion_cfstat_path' not in self.rawConfig.Main:
            self.checksLogger.debug('ColdFusion: cfstat path not in config, '
                                    'checking usual locations')
            cfstat = ''
            for cfstat_loc in cfstat_locs:
                if os.access(cfstat_loc, os.X_OK):
                    cfstat = cfstat_loc
                    break
            if not cfstat:
            ...
            if not os.access(cfstat, os.X_OK):
                self.checksLogger.error('ColdFusion: The location of cfstat '
                                        'given in config either does not '
        ...
        if int(pythonVersion[1]) >= 6:
            try:
                ...
                self.checksLogger.debug(
                    'ColdFusion: cfstat process already terminated')

        stats = stats.split()
        if len(stats) == 26:
            keys = ['Pg/s Now',     'Pg/s Hi',
                    'DB/s Now',     'DB/s Hi',
                    'CP/s Now',     'CP/s Hi',
                    'Reqs Q''ed',   'Reqs Run''g',   'Reqs TO''ed',
                    'Tpl Q''ed',    'Tpl Run''g',    'Tpl TO''ed',
            ...
            self.checksLogger.error('ColdFusion: Received unexpected response '
                                    'from cfstat: %s' % str(stats))
            return False
